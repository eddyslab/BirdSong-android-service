...
**최종 수정일 / Last Updated: 2026-07-06**
...
- **마이크(RECORD_AUDIO)**: 오디오 기반 메시지 송수신을 위해 필요합니다.
- **인터넷(INTERNET)**: 암호화된 데이터의 단축 URL 업로드/다운로드를 위해 필요합니다.
- **저장소/카메라(선택)**: 이미지 메시지를 전송하려는 경우에만 사용됩니다.
...
- **Microphone (RECORD_AUDIO)**: Required for sending/receiving audio-based messages.
- **Internet (INTERNET)**: Required to upload/download encrypted data via short URLs.
- **Storage/Camera (optional)**: Used only when sending image messages.
